...
const productInput = z.object({ categoryId: z.number().int().positive(), name: z.string().trim().min(2).max(160), slug, kind: z.enum(["VIP", "COINS", "KIT", "COSMETIC"]), priceCents: z.number().int().min(1).max(100_000_000), durationDays: z.union([z.literal(7), z.literal(15), z.literal(30), z.literal(90), z.literal(365)]).nullable().optional(), luckPermsGroup: z.string().trim().max(96).optional(), deliveryCommands: z.array(z.string().trim().min(1).max(255)).min(1).max(16), featured: z.boolean().default(false), serverIds: z.array(z.number().int().positive()).min(1).max(16) });
