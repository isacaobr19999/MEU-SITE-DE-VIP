  ...
  return (
    <div className="minecraft-store min-h-screen overflow-x-hidden bg-[#07111d] text-slate-100 selection:bg-emerald-300 selection:text-slate-950">
      ...
      <main className="relative z-10">
        ...
        {vipProducts.length ? <section id="comparar-vips" className="container py-16 sm:py-20"><div className="surface-panel overflow-hidden rounded-[1.8rem]"><div className="border-b border-white/10 p-6 sm:p-8"><p className="section-kicker">COMPARATIVO REAL</p><h2 className="minecraft-heading font-display mt-2 text-3xl font-bold text-white">Escolha seu VIP com clareza.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">A comparação usa somente duração, valor e detalhes já cadastrados nos produtos VIP ativos.</p></div><div className="grid gap-4 p-5 md:hidden">{vipProducts.map(product => <article key={`vip-card-${product.id}`} className="vip-compare-card">{bestVipValue?.id === product.id ? <span className="vip-value-badge">MENOR CUSTO/DIA</span> : null}<div className="flex items-start justify-between gap-3"><div><h3>{product.name}</h3><p>{product.durationDays ? `${product.durationDays} dias` : "Permanente"}</p></div><strong>{money.format(product.priceCents / 100)}</strong></div><ul>{getVipBenefits(product).length ? getVipBenefits(product).map(benefit => <li key={benefit}>{benefit}</li>) : <li>Detalhes disponíveis na página do produto.</li>}</ul><Link href={`/products/${product.slug}`}>Ver detalhes <ArrowUpRight size={14} /></Link></article>)}</div><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[760px] text-left"><thead className="bg-white/[.025] text-[10px] font-bold tracking-[.14em] text-slate-500"><tr><th className="px-6 py-4">VIP</th><th className="px-6 py-4">DURAÇÃO</th><th className="px-6 py-4">DETALHES CADASTRADOS</th><th className="px-6 py-4 text-right">VALOR</th><th className="px-6 py-4" /></tr></thead><tbody>{vipProducts.map(product => <tr key={`vip-${product.id}`} className="border-t border-white/[.07] text-sm"><td className="px-6 py-5 font-bold text-white">{product.name}{bestVipValue?.id === product.id ? <span className="vip-value-badge ml-2">MENOR CUSTO/DIA</span> : null}</td><td className="px-6 py-5 text-slate-300">{product.durationDays ? `${product.durationDays} dias` : "Permanente"}</td><td className="max-w-sm px-6 py-5 text-slate-400"><ul className="space-y-1">{getVipBenefits(product).length ? getVipBenefits(product).map(benefit => <li key={benefit} className="flex gap-2"><span className="text-emerald-300">◆</span>{benefit}</li>) : <li>Informação disponível na página do produto.</li>}</ul></td><td className="px-6 py-5 text-right font-mono font-bold text-emerald-200">{money.format(product.priceCents / 100)}</td><td className="px-6 py-5 text-right"><Link href={`/products/${product.slug}`} className="inline-flex items-center gap-1 text-xs font-bold text-emerald-200 hover:text-emerald-100">Ver VIP <ArrowUpRight size={14} /></Link></td></tr>)}</tbody></table></div></div></section> : null}
