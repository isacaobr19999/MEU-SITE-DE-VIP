...
export default function Login() {
  ...
  return <main className="minecraft-auth-page grid min-h-screen overflow-hidden px-4 py-8 text-slate-100 sm:place-items-center sm:p-8">
    <section className="minecraft-auth-panel relative mx-auto grid w-full max-w-5xl overflow-hidden sm:grid-cols-[.9fr_1.1fr]">
      <div className="minecraft-auth-story relative hidden overflow-hidden border-r border-white/10 p-10 text-slate-950 sm:flex sm:flex-col">
        ...
        <div className="relative my-auto"><p className="font-pixel text-xs font-bold tracking-[.18em] text-slate-700">CONTA DO JOGADOR</p><h1 className="minecraft-heading mt-4 text-4xl font-bold leading-tight">Acompanhe cada recompensa conquistada.</h1><p className="mt-5 max-w-sm text-sm leading-6 text-slate-700">Seu histórico, descontos e entregas ficam organizados em uma única conta vinculada à sua jornada.</p></div>
      ...
      <div className="p-6 sm:p-10"><Link href="/" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-400 transition hover:text-emerald-200 sm:hidden">← Voltar à loja</Link>
        <div className="mt-7 sm:mt-0"><BrandMark className="sm:hidden" /><p className="section-kicker mt-7 sm:mt-0">ACESSO SEGURO</p><h2 className="font-pixel mt-3 text-3xl font-bold text-white">{mode === "login" ? "Sua conta de jogador" : "Prepare sua conta"}</h2><p className="mt-2 max-w-md text-sm leading-6 text-slate-400">{mode === "login" ? "Entre para acompanhar benefícios, pedidos e entregas no servidor." : "Crie uma conta para manter sua progressão e histórico organizados."}</p></div>
        <div className="minecraft-auth-tabs mt-7 grid grid-cols-2 p-1 text-sm font-bold"><button type="button" onClick={() => { setMode("login"); setError(null); }} className={`px-3 py-2.5 transition ${mode === "login" ? "bg-emerald-200 text-slate-950 shadow-sm" : "text-slate-400 hover:text-white"}`}>Entrar</button><button type="button" onClick={() => { setMode("register"); setError(null); }} className={`px-3 py-2.5 transition ${mode === "register" ? "bg-emerald-200 text-slate-950 shadow-sm" : "text-slate-400 hover:text-white"}`}>Cadastrar</button></div>
