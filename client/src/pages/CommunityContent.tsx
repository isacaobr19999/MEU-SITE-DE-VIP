  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#07111d] text-slate-100 selection:bg-emerald-300 selection:text-slate-950">
    ...
    <main className="relative z-10 container pb-20 pt-14 sm:pt-20">
      <div className="surface-panel relative overflow-hidden rounded-[2rem] p-6 sm:p-10">
        ...
        <div className="relative mt-10 max-w-3xl"><div className="grid h-12 w-12 place-items-center rounded-2xl border border-emerald-300/20 bg-emerald-300/10 text-emerald-100"><Icon size={23} /></div><p className="section-kicker mt-6">{copy.eyebrow}</p><h1 className="font-display mt-3 text-4xl font-extrabold tracking-tight text-white sm:text-5xl">{copy.title}</h1><p className="mt-5 max-w-2xl text-base leading-8 text-slate-300">{copy.description}</p>{kind === "RULE" ? <p className="mt-4 max-w-2xl rounded-xl border border-emerald-300/15 bg-emerald-300/[.06] px-4 py-3 text-sm leading-6 text-emerald-100">Esta página acompanha as orientações oficiais publicadas no Discord, nos canais <strong>📜-regra</strong> e <strong>⚖️-regras-do-mercado</strong>.</p> : null}{kind === "POLICY" ? <p className="mt-4 max-w-2xl rounded-xl border border-amber-300/15 bg-amber-300/[.06] px-4 py-3 text-sm leading-6 text-amber-100">Os textos são administrados pela PlayStorCraft. Consulte a versão publicada antes de concluir uma compra.</p> : null}</div>
      </div>
      <section className="mt-10">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4"><h2 className="font-display text-2xl font-bold text-white">{kind === "RULE" ? "Orientações publicadas" : kind === "NEWS" ? "Publicações recentes" : "Políticas publicadas"}</h2><Link href={copy.alternatePath}><Button variant="outline" className="border-white/10 text-slate-200 hover:bg-white/[.06]">{copy.alternateLabel}</Button></Link></div>
