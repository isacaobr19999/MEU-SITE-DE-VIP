...
function MaintenanceLogin() {
  ...
  return <main className="grid min-h-screen place-items-center bg-[#07111d] px-5 py-8 text-slate-100"><section className="surface-panel w-full max-w-md rounded-[1.75rem] p-6 sm:p-8"><div className="grid h-12 w-12 place-items-center rounded-2xl border border-amber-300/25 bg-amber-300/[.08] text-amber-200"><LockKeyhole size={22} /></div><p className="section-kicker mt-6">ACESSO RESTRITO</p><h1 className="font-display mt-3 text-2xl font-bold text-white">Painel de manutenção</h1><p className="mt-3 text-sm leading-6 text-slate-400">Entre com o e-mail e a senha de uma conta administrativa para controlar a disponibilidade da loja.</p><form onSubmit={submit} className="mt-6 space-y-4"><label className="block"><span className="mb-2 block text-sm font-semibold text-slate-300">E-mail administrativo</span><Input required type="email" autoComplete="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="voce@email.com" /></label><label className="block"><span className="mb-2 block text-sm font-semibold text-slate-300">Senha</span><Input required minLength={10} type="password" autoComplete="current-password" value={password} onChange={event => setPassword(event.target.value)} placeholder="Sua senha" /></label>{error ? <p role="alert" className="rounded-xl border border-rose-300/20 bg-rose-300/[.07] px-3 py-2 text-sm text-rose-200">{error}</p> : null}<Button disabled={pending} className="w-full bg-emerald-300 text-slate-950 hover:bg-emerald-200">{pending ? <Loader2 className="animate-spin" /> : <LogIn size={17} />} Entrar no painel</Button></form><Link href="/" className="mt-5 inline-flex text-xs font-bold text-slate-500 hover:text-emerald-200">Voltar para a loja</Link></section></main>;
}
